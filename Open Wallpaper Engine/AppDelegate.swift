...
// MARK: Set Wallpaper Windows - One per screen
    ...
    /// The lock screen (and Mission Control) show the *system* desktop picture, not our
    /// desktop-level window, so it must be kept in sync with a static snapshot of the
    /// current wallpaper — otherwise the lock screen shows whatever was written last.
    func setPlacehoderWallpaper(with wallpaper: WEWallpaper) {
        // No wallpaper selected: restore the user's original macOS wallpaper. The
        // placeholder is type "video", so without this guard its "Wallpaper not found"
        // frame gets snapshotted onto the lock screen and sticks there.
        guard wallpaper.project != .invalid else {
        ...
        case "video":
            ...
            imageGenerator.generateCGImagesAsynchronously(forTimes: [NSValue(time: time)]) { [weak self] _, cgImage, _, _, error in
                if let error = error {
                    print(error)
                } else if let cgImage = cgImage {
                    self?.setStaticDesktopImage(NSImage(cgImage: cgImage, size: .zero),
                                                cacheKey: wallpaper.wallpaperDirectory.hashValue)
                }
            }
        default:
            // Scenes and images have no cheap "first frame" — use the project's preview
            // image (NSImage takes the first frame of an animated gif).
            guard !wallpaper.project.preview.isEmpty,
                  let image = NSImage(contentsOf: wallpaper.wallpaperDirectory.appending(component: wallpaper.project.preview))
            else { return }
            setStaticDesktopImage(image, cacheKey: wallpaper.wallpaperDirectory.hashValue)
        }
    }

    private func setStaticDesktopImage(_ image: NSImage, cacheKey: Int) {
        guard let data = image.tiffRepresentation else { return }
        do {
            let url = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0].appending(path: "staticWP_\(cacheKey).tiff")
